...
--   1Devuelve el listado de clientes indicando el nombre del cliente y cuántos pedidos ha realizado. Tenga en cuenta que pueden existir clientes que no han realizado ningún pedido.
select cliente.nombre_cliente,count(pedido.codigo_pedido) as pedidos from cliente left join pedido on pedido.codigo_cliente=cliente.codigo_cliente     
 group by cliente.nombre_cliente; 
--   2Devuelve un listado con los nombres de los clientes y el total pagado por cada uno de ellos. Tenga en cuenta que pueden existir clientes que no han realizado ningún pago.
select  cliente.nombre_cliente,sum(pago.total) as total_pagado from cliente left join pago on pago.codigo_cliente=cliente.codigo_cliente     
 group by cliente.nombre_cliente; 
--   3Devuelve el nombre de los clientes que hayan hecho pedidos en 2008 ordenados alfabéticamente de menor a mayor.
select distinct cliente.nombre_cliente , year(pedido.fecha_pedido)from cliente,pedido where year(pedido.fecha_pedido)=2008 order by cliente.nombre_cliente asc ;
--   4Devuelve el nombre del cliente, el nombre y primer apellido de su representante de ventas y el número de teléfono de la oficina del representante de ventas, de aquellos clientes que no hayan realizado ningún pago.
select distinct cliente.nombre_cliente,cliente.nombre_contacto,cliente.apellido_contacto,oficina.telefono from cliente,oficina,pago where pago.codigo_cliente=cliente.codigo_cliente and pago.id_transaccion=isnull(pago.id_transaccion); 
--   5Devuelve el listado de clientes donde aparezca el nombre del cliente, el nombre y primer apellido de su representante de ventas y la ciudad donde está su oficina.

--   6Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que no sean representante de ventas de ningún cliente.

--   7Devuelve un listado indicando todas las ciudades donde hay oficinas y el número de empleados que tiene.
select * from cliente;